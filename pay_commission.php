...
<body>
	...
	<div id="page-wrap">
		...
		<h3>::Panel for paying Staff their commission</h3>
...
?>
   ...
   <input type="text" name="from" size="30" id="from" readonly="readonly" /><input name="DateFormats" id="DateFormats" disabled value="dd/mm/yyyy" type="hidden">
	...
	<td width="250"><div align="center"><strong>Commision Balance (Kshs)</strong></td>

	<td width="50"><div align="center"><strong>Pay</strong></td>
...
{  
$sql="select users.user_id,employees.emp_firstname,employees.emp_lastname,employees.emp_middle_name,SUM(commisions_expected.tll_commision) 
as all_com_exp,commisions_expected.invoice_no,commisions_expected.date_generated,
commisions_expected.sales_code,commisions_expected.curr_rate,commisions_expected.user_id,currency.curr_name from 
currency,users,commisions_expected,employees where users.emp_id=employees.emp_id and commisions_expected.user_id=users.user_id 
and commisions_expected.currency_code=currency.curr_id group by commisions_expected.user_id order by commisions_expected.date_generated desc";
...
echo '<tr bgcolor="#FFFFCC" height="25" >';
 ...
 ?>
    ...
    <!--<td ><?php echo $rows->curr_name.' '.number_format($ttl_com=$rows->all_com_exp,2);?></td>-->
...
<td align="right"><strong><font color="#009900"><?php 
	...
	$sqlrecpd="select SUM(commision_paid.amount_paid) as ttl_amnt_compaid from commision_paid where commision_paid.sales_rep='$user'";
    $resultsrecpd= mysql_query($sqlrecpd) or die ("Error $sqlrecpd.".mysql_error());
	  
	$rowsrecpd=mysql_fetch_object($resultsrecpd);
	  
	$ttlrecpd=$rowsrecpd->ttl_amnt_compaid;
...
?></font></strong></td>
	...
	$comm_bal=$ttl_rec_kshs-$ttl_recpd_kshs;
	
	//$ttl_combal_kshs=$curr_rate*$comm_bal;
	
	echo number_format($comm_bal,2);
	
?></font></strong></td>

	<td align="center" width="100">
	<?php
if ($comm_bal==0 && $ttl_rec_kshs==0 )
{

}
elseif ($comm_bal!=0 && $ttl_rec_kshs!=0 )
{
?>
<a href="pay_staff_comm2.php?comm_due=<?php echo $ttl_rec_kshs; ?>&comm_paid=<?php echo $ttl_recpd_kshs;?>&comm_balance=<?php echo $comm_bal;?>&sales_rep=<?php echo $rows->user_id;?>">Pay Balance</a>
<?php 
}

elseif ($ttl_rec_kshs==$ttl_recpd_kshs && $ttl_recpd_kshs==0 )
{
?>
<a href="pay_staff_comm2.php?comm_due=<?php echo $ttl_rec_kshs; ?>&sales_rep=<?php echo $rows->user_id; ?>">Pay</a>
<?php 
}
elseif ($ttl_rec_kshs==$ttl_recpd_kshs && $ttl_recpd_kshs!='0')
{
?>
<font color="#ff0000" size="-1">Cleared..</font>
<?php 
}
...

{
  ...
  <?php 
 $grnd_ttl_rec_kshs=$grnd_ttl_com_kshs+$ttl_rec_kshs;
$grnd_ttl_recpd_kshs=$grnd_ttl_recpd_kshs+$ttl_recpd_kshs;
$grnd_comm_bal=$grnd_comm_bal+$comm_bal;
  }
  ?>
  <tr  height="30" bgcolor="#FFFFCC">
	...
	<td width="150"><div align="right"><strong><?php echo number_format($grnd_ttl_recpd_kshs,2); ?></strong></td>
	<td width="200"><div align="right"><strong><?php echo number_format($grnd_comm_bal,2); ?></strong></td>
	<td width="300"><div align="right"><strong><?php //echo number_format($grnd_ttl_rec_kshs,2); ?></strong></td>
