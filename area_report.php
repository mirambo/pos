...
<input type="text" name="reg_no" size="40" /><strong>Or By Task Name:</strong>
  ...
  <tr  style="background:url(images/description.gif);" height="30" >
    ...
    <td align="center" width="150"><strong>Time Clocked In</strong></td>
    <td align="center" width="150"><strong>Cock In/Clock Out</strong></td>
    <td align="center" width="150"><strong>Stop /Resume Task</strong></td>
	<td align="center" width="100"><strong>Finish Task</strong></td>
...
$sql="SELECT * FROM job_card_task where finished_status='0' order by date_generated desc";
...
if ($reg_no!='' && $task_name=='')
{
$sql="SELECT * FROM job_card_task,bookings where job_card_task.booking_id=bookings.booking_id AND finished_status='0' AND 
...
if ($reg_no=='' && $task_name!='')
{
$sql="SELECT * FROM job_card_task.task where job_card_task.task_id=task.task_id AND 
...
$sql="SELECT * FROM job_card_task where finished_status='0' AND job_card_task.technician_id='$user_id' order by date_generated desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
else
{
...
if ($reg_no!='' && $task_name=='')
{
$sql="SELECT * FROM job_card_task,bookings where job_card_task.booking_id=bookings.booking_id AND finished_status='0' AND job_card_task.technician_id='$user_id' AND 
...
if ($reg_no=='' && $task_name!='')
{
$sql="SELECT * FROM job_card_task.task where job_card_task.task_id=task.task_id AND job_card_task.technician_id='$user_id' AND 
task.task_name LIKE '%$task_name%' order by job_card_task.date_generated desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
...
echo $time_clocked_id=$rowscl->date_clocked;	
...
//$assigned_job_card_id=$rows->assigned_job_card_id;

/* $sqlc="SELECT * FROM job_card_clock where assigned_job_card_id='$assigned_job_card_id' AND clock_type='i' order by job_card_clock_id desc limit 1";
$resultsc= mysql_query($sqlc) or die ("Error $sqlc.".mysql_error());
$rowsc=mysql_fetch_object($resultsc); */

 $in_status=$rows->clocked_in_status;
 $out_status=$rows->clocked_out_status;
 $finished_status=$rows->finished_status;

if ($in_status==1 && $out_status==0 && $finished_status==0)
{?>
<font color="#006600"><strong>In Progress..</strong></font>
<?php
}

if ($in_status==1 && $out_status==1 && $finished_status==0)
{?>
<font color="#006600"><strong>In Progress..</strong></font>
<?php
}
...
if ($in_status==0 && $out_status==0 && $finished_status==0)
{?>
<a href="home.php?clock=clock&job_card_task_id=<?php echo $rows->job_card_task_id; ?>">Clock In</a>
<?php 
}
	?>
	</td>
...
if ($in_status==1 && $out_status==0 && $finished_status==0)
{?>
<a href="home.php?stopclock=stopclock&job_card_task_id=<?php echo $rows->job_card_task_id; ?>">Stop Job Card</a>
<?php
...
if ($in_status==1 && $out_status==1 && $finished_status==0)
{?>
<a style="color:#ff0000; font-weight:bold;" href="resume_task.php?job_card_clock_id=<?php echo $rowsc3->job_card_clock_id; ?>&job_card_task_id=<?php echo $rows->job_card_task_id; ?>" onClick="return confirmDelete();">Resume Job Card>></a>
<?php
...
{
?>
<a href="home.php?clockout=clockout&job_card_task_id=<?php echo $rows->job_card_task_id; ?>">Clock Out Job Card</a>
	<?php
