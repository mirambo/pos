...
$queryprof="select * from  recovered_bad_debts where sales_code='$sales_code' and  client_id ='$client_id' and amount='$amount' and reason='$reason'";
...
$sql= "insert into recovered_bad_debts values ('','$sales_code','$client_id','$amount','$reason','$currency_code','$curr_rate',NOW(),'')";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());

//$sqlpdt="update bad_debts set status='1' where sales_code='$sales_code'";
...
$transaction_desc='Recovered Bad Debts Inv:'.$invoice_no;
...
$sqltrans= "insert into client_transactions values('','$client_id','$sales_code','Bad Debt Recovery for Inv: $invoice_no','$amount',NOW())";
$resultstrans=mysql_query($sqltrans) or die ("Error $sqltrans.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Cash for Bad Debt Recovery for Inv: $invoice_no','$amount','$amount','','$currency_code','$curr_rate',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Accounts Receivables(Bad Debt Recovery Inv No:$invoice_no)','-$amount','','$amount','$currency_code','$curr_rate',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

$sqlaccpay= "insert into cash_ledger values('','Cash (Bad Debt Recovery for Inv: $invoice_no)','$amount','$amount','','$currency_code','$curr_rate',NOW())";
$resultsaccpay=mysql_query($sqlaccpay) or die ("Error $sqlaccpay.".mysql_error());

$sqlaccpay= "insert into accounts_receivables_ledger values('','Bad Debt Recovery for Inv: $invoice_no','-$amount','','$amount','$currency_code','$curr_rate',NOW(),'$sales_code')";
$resultsaccpay=mysql_query($sqlaccpay) or die ("Error $sqlaccpay.".mysql_error());
...
header ("Location:recover_bad_debts.php?success=1");
