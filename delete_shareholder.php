...
$queryexam="select * from  shares where shares_id='$id'";
$resultsexam=mysql_query($queryexam) or die ("Error: $queryexam.".mysql_error());
$rowsexam=mysql_fetch_object($resultsexam);
$exit_shares_amnt=$rowsexam->shares_amount;
$share_holder=$rowsexam->share_holder_name;


$sqlexit= "delete from shares where shares_id='$id'";
$resultsexit= mysql_query($sqlexit) or die ("Error $sqlexit.".mysql_error());


/* $sqlgenled= "insert into general_ledger values('','Cash Exit of Shareholder $share_holder','-$exit_shares_amnt','','$exit_shares_amnt','6','1',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Exit of Shareholder $share_holder','$exit_shares_amnt','$exit_shares_amnt','','6','1',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());
 */

$sqlexit= "delete from shares_ledger where order_code='capt$id'";
$resultsexit= mysql_query($sqlexit) or die ("Error $sqlexit.".mysql_error());

$sqlexit= "delete from cash_ledger where sales_code='capt$id'";
$resultsexit= mysql_query($sqlexit) or die ("Error $sqlexit.".mysql_error());

$sqlexit= "delete from bank_ledger where sales_code='capt$id'";
$resultsexit= mysql_query($sqlexit) or die ("Error $sqlexit.".mysql_error());

$sqlexit= "delete from bank_statement where sales_code='capt$id'";
$resultsexit= mysql_query($sqlexit) or die ("Error $sqlexit.".mysql_error());

$sqlexit= "delete from shareholder_transactions where order_code='capt$id'";
$resultsexit= mysql_query($sqlexit) or die ("Error $sqlexit.".mysql_error());

$sqldel= "UPDATE shares SET status='1' where shares_id='$id'";
