...
<?php require_once('includes/supplier_refund_submenu.php');  ?>
		
		<h3>::List Of All Supplier Refund Recorded</h3>
...
?>
	...
	<tr bgcolor="#FFFFCC">
	<td colspan="11" align="center"><strong>Search : Supplier:</strong>
...
<a target="_blank" style="float:right; margin-right:100px; font-weight:bold; font-size:13px; color:#000000" href="print_list_supplier_refund.php?client=<?php echo $client?>&date_from=<?php echo $date_from ?>&date_to=<?php echo $date_to ?>">Print</a>						  
...
$sql="select * FROM mop,supplier_refund,currency where 
supplier_refund.mop=mop.mop_id AND supplier_refund.currency_code=currency.curr_id ORDER BY supplier_refund.date_received desc";
...
//echo "client only";
$sql="select * FROM mop,supplier_refund,currency where 
supplier_refund.mop=mop.mop_id AND supplier_refund.currency_code=currency.curr_id and supplier_refund.supplier_id='$client' order by supplier_refund.date_paid  desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
...
//echo "Dea only";
$sql="SELECT * FROM mop,supplier_refund,currency where 
supplier_refund.mop=mop.mop_id AND supplier_refund.currency_code=currency.curr_id and supplier_refund.date_paid BETWEEN '$date_from' AND '$date_to' order by supplier_refund.date_paid  desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
elseif($client!=0 && $date_from!='' && $date_to!='')
{
...
$sql="SELECT * FROM mop,supplier_refund,currency where 
supplier_refund.mop=mop.mop_id AND supplier_refund.currency_code=currency.curr_id and 
supplier_refund.date_paid BETWEEN '$date_from' AND '$date_to' and supplier_refund.supplier_id='$client' order by supplier_refund.date_paid  desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
...
echo $date_received=$rows->date_paid; 
	...
	$client=$rows->supplier_id;
	
$queryprod="select * from suppliers where supplier_id='$client'";
$resultsprod=mysql_query($queryprod) or die ("Error: $queryprod.".mysql_error());
$rowsprod=mysql_fetch_object($resultsprod); 
echo $exp_cat_name=$rowsprod->supplier_name;
...
else
{
	...
	<td align="center"><a href="edit_supplier_refund.php?supplier_refund_id=<?php echo $rows->supplier_refund_id;?>"><img src="images/edit.png"></a></td>
    <td align="center"><a href="delete_supplier_refund.php?supplier_refund_id=<?php echo $rows->supplier_refund_id;?>"  onClick="return confirmDelete();"><img src="images/delete.png"></a></td>
