...
$rowsemp_det=mysql_fetch_object($resultsemp_det);
	 $f_name=$rowsemp_det->emp_firstname;
	  $m_name=$rowsemp_det->emp_middlename;
...
if ($_GET['next']==1)
{
echo '<div align="center" style="background: #FFCC33; height:20px; width:550px; border:#FF0000 solid 1px; font-size:11px;" class="br-5"> <p align="center"><font color="#FF0000" >Payroll for' ;?> <?php echo $f_name.' '.$m_name.' '.$l_name;?> <?php echo 'has been approved successfully</font></strong></p></div>';
}
?> 
...
employees.employee_no,employees.basic_sal FROM payroll,employees WHERE payroll.emp_id=employees.emp_id AND payroll.status='0' order by payroll.payroll_id desc";
...
$sql="select payroll.payroll_id,payroll.payment_month,payroll.salary_advance,payroll.payroll_no,payroll.unpaid_balance,payroll.comm_due,
payroll.emp_id,payroll.net_pay,employees.emp_firstname,employees.emp_middle_name,employees.emp_lastname,
employees.employee_no,employees.basic_sal FROM payroll,employees WHERE payroll.emp_id=employees.emp_id AND payroll.status='0' AND payroll.emp_id='$emp' order by payroll.payroll_id desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
...
$sql="select payroll.payroll_id,payroll.payment_month,payroll.salary_advance,payroll.payroll_no,payroll.unpaid_balance,payroll.comm_due,
payroll.emp_id,payroll.net_pay,employees.emp_firstname,employees.emp_middle_name,employees.emp_lastname,
employees.employee_no,employees.basic_sal FROM payroll,employees WHERE payroll.emp_id=employees.emp_id AND payroll.status='0' AND payroll.payment_month='$current_month' order by payroll.payroll_id desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
...
$sql="select payroll.payroll_id,payroll.payment_month,payroll.salary_advance,payroll.payroll_no,payroll.unpaid_balance,payroll.comm_due,
payroll.emp_id,payroll.net_pay,employees.emp_firstname,employees.emp_middle_name,employees.emp_lastname,
employees.employee_no,employees.basic_sal FROM payroll,employees WHERE payroll.emp_id=employees.emp_id AND payroll.status='0' AND payroll.emp_id='$emp' AND payroll.payment_month='$current_month' order by payroll.payroll_id desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
else
{
...
echo number_format($paid_sal=$sal_paid*$curr_rate,2);
	...
	<!-- <a href="view_payslip.php?payroll_id=<?php echo $payroll_id; ?>" target="_blank">View Details</a>-->
	<a href="approve_payslip.php?payroll_id=<?php echo $payroll_id; ?>&approve=1">Approve..</a>
