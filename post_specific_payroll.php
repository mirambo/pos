...
$queryopr="SELECT *,hs_hr_employee.employee_id as emp_num FROM payroll_code,hs_hr_employee where payroll_code.employee_id=hs_hr_employee.emp_number 
...
<table width="90%" border="0" align="center" bgcolor="#9AD7AF" style="border-radius:5px;">
  ...
  <tr height="20">
    <td width="10%">&nbsp;</td>
    <td width="13%">Select Employee Name<font color="#FF0000">*</font></td>
    <td width="10%">
...
<option value="<?php echo $rows1n->emp_number; ?>"><?php echo $rows1n->emp_num.' - '.$rows1n->emp_firstname.' - '.$rows1n->emp_firstname.' '.$rows1n->emp_father_name.' '.$rows1n->emp_lastname; ?></option>
...
if ($next_employee_number!='')
	...
	else
	{
			...
			$query1="select *,hs_hr_employee.employee_id as staff_no from hs_hr_employee,employee_job_details WHERE 
								  hs_hr_employee.emp_number=employee_job_details.employee_id AND 
								  employee_job_details.terms_of_service='2' order by hs_hr_employee.employee_id asc";
								  ...
								  
								  {
                                    ...
                                    <option value="<?php echo $rows1->emp_number; ?>"><?php echo $rows1->staff_no.' - '.$rows1->emp_firstname.' '.$rows1->emp_father_name.' '.$rows1->emp_lastname; ?> </option>
...
if ($prc=='')
  ...
  <tr height="20">
    ...
    <td width="10%"><!--Bonus<font color="#FF0000">*</font>--></td>
    <td width="30%"><!--<input class="form-control" size="41" type='text'  value="<?php echo $rowsr->bonus;?>" name='bonus'/>--></td>
    <td width="10%"></td>
  </tr>
  
    <tr height="20">
    <td bgcolor="">&nbsp;</td>
    <td width="1%">Days Worked<font color="#FF0000">*</font></td>
    <td width="10%"><input  class="form-control" style="width:30%;" size="41" type='number'  required value="<?php echo $rowsr->days_worked; ?>" name="days_worked"/><span>
	
	<?php 

$sql3="select rate_amount from specific_daily_rate";
$results3= mysql_query($sql3) or die ("Error $sql3.".mysql_error());
$rows3=mysql_fetch_object($results3);

	?>
	Daily Rate : <input  class="form-control" style="width:30%;" type='text'  readonly value="<?php echo $rows3->rate_amount; ?>" name="daily_rate"/>
