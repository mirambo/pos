...
$_SESSION['curr_rate']=$curr_rate;
$current_month=(Date("F Y"));

$grndttl=$_SESSION['grndttl'];
session_start();
...
//$sql= "insert into invoice_payments values ('','$get_prnt_sales_id','$get_prnt_client_id','$cash_rec','$currency','$curr_rate','$mop',NOW(),'')";
...
$sqlreccom= "insert into commision_payments values ('','$get_prnt_sales_id','$sales_rep','$comm_paid','$currency','$curr_rate',NOW(),'$current_month','','')";
...
{
$sqllpo= "insert into invoices values('','','','','$currency','$curr_rate','$get_prnt_client_id','$latest_sales_code','$user_id','$sess_sales_rep',NOW(),'1','1')";
...
$sqlgrndttl= "update invoices set invoice_ttl='$grndttl',inv_bal='$grndttl' where sales_code='$latest_sales_code'";
...
$transaction_descinv='Inv No:'.$invoice_no;
$transaction_descrec='Cash Receipt No:'.$invoice_no;
...
$sqltrans= "insert into client_transactions values('','$get_prnt_client_id','$latest_sales_code','$transaction_descinv','$grndttl',NOW())";
$resultstrans=mysql_query($sqltrans) or die ("Error $sqltrans.".mysql_error());

$sqltrans= "insert into client_transactions values('','$get_prnt_client_id','$latest_sales_code','$transaction_descrec','-$cash_rec',NOW())";
$resultstrans=mysql_query($sqltrans) or die ("Error $sqltrans.".mysql_error());
...
$sqlgenled= "insert into general_ledger values('','Sales ( Against Inv No:$invoice_no)','-$grndttl','','$grndttl','$currency','$curr_rate',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Accounts Receivables (Inv No:$invoice_no)','$grndttl','$grndttl','','$currency','$curr_rate',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

//Record Cash sales now
$sqlgenled= "insert into general_ledger values('','Cash ( Against Inv No:$invoice_no)','$cash_rec','$cash_rec','','$currency','$curr_rate',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Accounts Receivables (Inv No:$invoice_no)','-$cash_rec','','$cash_rec','$currency','$curr_rate',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());


$sqlgenled= "insert into cash_ledger values('','Sale Payment against (Inv No:$invoice_no)','$cash_rec','$cash_rec','','$currency','$curr_rate',NOW())";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());
...
header ("Location:generate_invoice_back.php?sales_code=$latest_sales_code&client_id=$get_prnt_client_id&currency=$get_prnt_currency&saveconfirm=1");
