...
/*$sql= "insert into cancelled_invoices values('','$invoice_no','$sales_code','$reasons','0')";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());*/

$querycancel="UPDATE invoices set status='1' where invoice_id='$invoice_id'";
$resultscancel=mysql_query($querycancel) or die ("Error: $querycancel.".mysql_error());

$querycancelsale="UPDATE sales set status='1' where sales_code='$sales_code'";
$resultscancelsale=mysql_query($querycancelsale) or die ("Error: $querycancelsale.".mysql_error());

$querycancelprod="UPDATE sales set status='1' where sales_code='$sales_code'";
$resultscancelprod=mysql_query($querycancelprod) or die ("Error: $querycancelprod.".mysql_error());

$querycancelprod="UPDATE commisions_expected set status='1' where sales_code='$sales_code'";
$resultscancelprod=mysql_query($querycancelprod) or die ("Error: $querycancelprod.".mysql_error());
...
$transaction_descinv="Recovery Of Invoice $invoice_no";


$sqltrans= "insert into client_transactions values('','$client_id','$sales_code','$transaction_descinv','$grndttl',NOW())";
$resultstrans=mysql_query($sqltrans) or die ("Error $sqltrans.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Recovered Cancelled Sales ( Against Inv No:$invoice_no)','-$grndttl','','$grndttl','$currency','$curr_rate',NOW(),'')";
...
$sqlauditsav= "insert into audit_trail values('','$user_id',NOW(),'Recovery of Invoice $invoice_no')";
...
header ("Location:view_cancelled_invoices.php? recoverconfirm=1");
