...
$queryprof="select * from  sipet_invoice_payments where month_generated='$month_generated' and  client_id ='$client_id' and amount_received='$amnt_paid' and mop='$mop'";
...
header ("Location:home.php?recordpayment=recordpayment&project_id=$project_id&client_id=$client_id&month_generated=$month_generated&invoice_id=$invoice_id&invoice_ttl=$invoice_ttl&recordexist=1");
...
header ("Location:home.php?recordpayment=recordpayment&abnormal=1&invoice_id=$invoice_id&balance=$bal&amnt_paid=$amnt_paid&amnt_rec_sofar=$amnt_rec_sofar&change=$change");
...
$sql= "insert into sipet_invoice_payments values ('','$month_generated','$invoice_id','$project_id','$client_id','$amnt_paid','$mop',NOW(),'')";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());



$transaction_desc='Received Payment for Invoice No:'.$invoice_no;
...
$transaction_desclg='Received Payment for inv:'.$invoice_no;
...
$sqlgenled= "insert into general_ledger values('','Accounts Receivables (Invoice No : $invoice_no)','-$amnt_paid','','$amnt_paid','2','1',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Cash Received ( Invoice No : $invoice_no)','$amnt_paid','$amnt_paid','','2','1',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());
...
header ("Location:home.php?receivepay=receivepay");
}
