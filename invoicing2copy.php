...
?>
  ...
  <tr  style="background:url(images/description.gif);" height="30">
	...
	<?php 
	$last_date_invoiced=$_GET['last_invoiced_date'];
	list($date, $time) = explode(" ", $last_date_invoiced);
    $date;
   if ($date!='')
   {
    ...
    $grnd_ttl_pdm1=0;
	$sql_pm="SELECT bunch.project_manager_id,bunch.bunch_id, employees.emp_firstname, employees.emp_middle_name,employees.emp_lastname,
	...
	AND employees.job_title_id=job_titles.job_title_id AND bunch.client_id='$client_id' AND employees.emp_id=bunch.project_manager_id 
	AND bunch.period_from<=CURDATE() AND bunch.period_from BETWEEN '$date' AND '$curr_date'";
...
echo $days_remainingpm;
 ...
 $grnd_ttl_pdm=0;
$sql="SELECT employees.department,employees.emp_firstname,employees.emp_id,employees.emp_middle_name,employees.emp_lastname,
employees.job_cat_id,job_category.job_cat_name ,blocks.block_name,bunch.period_from,bunch.period_to,bunch.bunch_id,bunch.project_manager_id,
clients.c_name,assigned_staff.status,assigned_staff.emp_id as ass_emp_id FROM employees,job_category,assigned_staff,bunch,blocks,
clients where employees.job_cat_id=job_category.job_cat_id AND assigned_staff.emp_id=employees.emp_id AND 
assigned_staff.bunch_id=bunch.bunch_id AND  bunch.block_id=blocks.block_id AND clients.client_id=bunch.client_id AND 
bunch.period_to>=CURDATE() AND CURDATE()>=bunch.period_from  AND bunch.client_id='$client_id' order by blocks.block_name,assigned_staff.assigned_staff_id asc";
...
echo $days_remaining;
  ...
  <td></td>
  <td align="right"><font size="+1" color="#ff0000"><strong><?php echo number_format($ttl_supper_ttl=$supper_grnd_ttl1+$supper_grnd_ttl,2);
 ;
  ?></strong></font></td>
  </tr>
  <tr bgcolor="#FFFFCC">
  ...
  <td colspan="7" align="center"><p>Print Invoice</p><a href="pre_invoice.php?client_id=<?php echo $client_id;?>&bunch_id=<?php echo $bunch_id; ?>&invoice_amount=<?php echo $ttl_supper_ttl;?>" target="_blank"><img src="images/print_icon.gif" title="Print"></a> &nbsp; <a href="printsubspecialitycsv.php"><img src="images/excel.png" title="Export to Excell"></a> &nbsp; <a href="printsubspecialityword.php"><img src="images/word.png" title="Export to Word"></a></td>
  </tr>
  <?php 
