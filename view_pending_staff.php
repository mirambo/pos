...
$sql="SELECT projects.project_id,temp_assigned_staffold.project_manager,operations.operation_name,clients.c_name,employees.emp_id,employees.staff_type,employees.emp_fname,employees.emp_mname,
employees.emp_lname,omjob_titles.omjob_title_name,job_category.job_cat_name,temp_assigned_staffold.temp_assigned_staff_id,temp_assigned_staffold.entry_date,temp_assigned_staffold.exit_date,temp_assigned_staffold.work_place,temp_assigned_staffold.segment,temp_assigned_staffold.staff FROM operations, projects,
temp_assigned_staffold,employees,clients,omjob_titles,job_category WHERE job_category.job_cat_id=employees.staff_type AND omjob_titles.omjob_title_id=employees.title AND projects.client_id=clients.client_id AND temp_assigned_staffold.project_id=projects.project_id AND  operations.operation_id=projects.operation_id AND temp_assigned_staffold.project_id=projects.project_id 
and  employees.emp_id=temp_assigned_staffold.staff order by clients.c_name asc";
...
$ass_emp_id=$rows->ass_emp_id;
$project_manager_id=$rows->project_manager_id;
if ($days_remaining<='1')
{

$sqlupdemp= "UPDATE employees SET status='0' where emp_id='$ass_emp_id'";
...
$sqlupdpm= "UPDATE temp_assigned_staff SET status='1' where emp_id='$ass_emp_id'";
$resultsupdpm= mysql_query($sqlupdpm) or die ("Error $sqlupdpm.".mysql_error());
...
?></font></strong></td>
	...
	elseif ($days_remaining<30 && $status!='r')
	{?>
