...
header ("Location:invoice.php?contract_no=$profcontract_no&client_id=$profclient_id&invoice_no=$profinvoice_no&invoice_amount=$profinvoice_ttl&ttl_days=$profdays_spent&manhours_charges=$profmanhours_amount&visa_charges=$profvisa_charges&per_dm=$profper_dem&flight_charges=$profflight_charges");
...
$sqltrans= "insert into client_transactions values('','$client_id','Service invoice $invoice_no for month $current_month','$invoice_amount',NOW())";
$resultstrans=mysql_query($sqltrans) or die ("Error $sqltrans.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Service invoice($invoice_no) for month $current_month','-$invoice_amount','','$invoice_amount','$currency','$curr_rate',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

$sqlgenled= "insert into general_ledger values('','Accounts Receivables invoice $invoice_no for month $current_month','$invoice_amount','$invoice_amount','','$currency','$curr_rate',NOW(),'')";
$resultsgenled=mysql_query($sqlgenled) or die ("Error $sqlgenled.".mysql_error());

$sqlaccpay= "insert into accounts_receivables_ledger values('','Service invoice $invoice_no for month $current_month','$invoice_amount','$invoice_amount','','$currency','$curr_rate',NOW(),'')";
$resultsaccpay=mysql_query($sqlaccpay) or die ("Error $sqlaccpay.".mysql_error());

$sqlaccpay= "insert into sales_ledger values('','Service invoice($invoice_no) for month $current_month','$invoice_amount','','$invoice_amount','$currency','$curr_rate',NOW())";
$resultsaccpay=mysql_query($sqlaccpay) or die ("Error $sqlaccpay.".mysql_error());
...
header ("Location:invoice.php?contract_no=$contract_no&client_id=$client_id&invoice_no=$invoice_no&invoice_amount=$invoice_amount&ttl_days=$ttl_days&manhours_charges=$manhours_charges&visa_charges=$visa_charges&per_dm=$per_dm&flight_charges=$flight_charges");
