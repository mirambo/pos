...
<body>
	...
	<?php require_once('includes/suppliersubmenu.php');  ?>
		...
		<h3>:: Statement of Account for  Farmer : <?php 
		$sqlclient="select * from farmers where farmer_id='$id'";
		$resultsclient= mysql_query($sqlclient) or die ("Error: $sqlclient.".mysql_error());
        $rowsclient=mysql_fetch_object($resultsclient);
		
		echo $rowsclient->farmer_name;
...
<table width="100%" border="0">
  <tr bgcolor="#FFFFCC">
	...
	Statement of Account for  Farmer </font> <font size="+1" color="#FF6600"><?php 
		$sqlclient="select * from farmers where farmer_id='$id'";
		$resultsclient= mysql_query($sqlclient) or die ("Error: $sqlclient.".mysql_error());
        $rowsclient=mysql_fetch_object($resultsclient);
...
$sql="select * from supplier_transactions where farmer_id='$id' order by transaction_date asc";
...
$sql="select * from supplier_transactions where farmer_id='$id' and transaction_date>='$date_from' 
AND transaction_date<='$date_from' order by transaction_date asc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
