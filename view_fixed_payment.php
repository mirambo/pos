...
<?php require_once('includes/fixedassetsubmenu.php');  ?>
		...
		<h3>::List Of All Fixed Assets Payment Transactions</h3>
...
?>
	...
	<tr bgcolor="#FFFFCC">
	<td colspan="11" align="center"><strong>Search : By Fixed Asset:</strong>
	<select name="client">
	<option value="0">-------------------Select-----------------</option>
								  <?php
								  
								  $query1="select * from fixed_assets order by fixed_asset_name asc";
								  ...
								  
								  {
                                    ...
                                    <option value="<?php echo $rows1->fixed_asset_id; ?>"><?php echo $rows1->fixed_asset_name; ?> </option>
...
<a target="_blank" style="float:right; margin-right:100px; font-weight:bold; font-size:13px; color:#000000" href="print_list_fixed_assets_payment.php">Print</a>						  
   ...
   <tr style="background:url(images/description.gif);" height="30" >
    <td width="10%"><div align="center"><strong>Asset Name</strong></td>
...
$sql="select * FROM fixed_assets,mop,fixed_assets_payments,currency where 
fixed_assets_payments.mop=mop.mop_id AND fixed_assets.fixed_asset_id=fixed_assets_payments.fixed_asset_id AND 
fixed_assets_payments.currency_code=currency.curr_id ORDER BY fixed_assets_payments.date_received desc";
...
$sql="select * FROM fixed_assets,mop,fixed_assets_payments,currency where 
fixed_assets_payments.mop=mop.mop_id AND fixed_assets.fixed_asset_id=fixed_assets_payments.fixed_asset_id AND 
fixed_assets_payments.currency_code=currency.curr_id AND fixed_assets.fixed_asset_id='$client' ORDER BY fixed_assets_payments.date_received desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
...
$sql="select * FROM fixed_assets,mop,fixed_assets_payments,currency where 
fixed_assets_payments.mop=mop.mop_id AND fixed_assets.fixed_asset_id=fixed_assets_payments.fixed_asset_id AND 
fixed_assets_payments.currency_code=currency.curr_id AND fixed_assets_payments.date_paid BETWEEN '$date_from' AND '$date_to' ORDER BY fixed_assets_payments.date_received desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
elseif($client!='0' && $date_from!='' && $date_to!='')
{
...
$sql="select * FROM fixed_assets,mop,fixed_assets_payments,currency where 
fixed_assets_payments.mop=mop.mop_id AND fixed_assets.fixed_asset_id=fixed_assets_payments.fixed_asset_id AND 
fixed_assets_payments.currency_code=currency.curr_id AND fixed_assets_payments.date_paid BETWEEN '$date_from' AND '$date_to' 
AND fixed_assets.fixed_asset_id='$client' ORDER BY fixed_assets_payments.date_received desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
...
$mop=$rows->mop;
	...
	elseif ($mop==2)//cheque
	 {
	echo $rows->mop_name.'</br>'.'<i>(Cheque No:'.$rows->cheque_no.')</i>';
	}
...
else
{
	...
	<td align="center"><a href="edit_fixed_assets_payments.php?fixed_asset_repayment_id=<?php echo $rows->fixed_asset_repayment_id;?>"><img src="images/edit.png"></a></td>
    <td align="center"><a href="delete_fixed_assets_payments.php?fixed_asset_repayment_id=<?php echo $rows->fixed_asset_repayment_id;?>"  onClick="return confirmDelete();"><img src="images/delete.png"></a></td>
