...
<?php require_once('includes/customer_refund_submenu.php');  ?>
		
		<h3>::List Of All Customer Refund Recorded</h3>
...
?>
	...
	<tr bgcolor="#FFFFCC">
	<td colspan="11" align="center"><strong>Search : Customer:</strong>
...
<a target="_blank" style="float:right; margin-right:100px; font-weight:bold; font-size:13px; color:#000000" href="print_list_customer_refund.php?client=<?php echo $client?>&date_from=<?php echo $date_from ?>&date_to=<?php echo $date_to ?>">Print</a>						  
   ...
   <tr style="background:url(images/description.gif);" height="30" >
	...
	<td width="10%"><div align="center"><strong>Customer</strong></td>
...
$sql="select * FROM mop,customer_refund,currency where 
customer_refund.mop=mop.mop_id AND customer_refund.currency_code=currency.curr_id ORDER BY customer_refund.date_received desc";
...
//echo "client only";
$sql="select * FROM mop,customer_refund,currency where 
customer_refund.mop=mop.mop_id AND customer_refund.currency_code=currency.curr_id and customer_refund.client_id='$client' order by customer_refund.date_paid  desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
...
//echo "Dea only";
$sql="SELECT * FROM mop,customer_refund,currency where 
customer_refund.mop=mop.mop_id AND customer_refund.currency_code=currency.curr_id and customer_refund.date_paid BETWEEN '$date_from' AND '$date_to' order by customer_refund.date_paid  desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
elseif($client!=0 && $date_from!='' && $date_to!='')
{
...
$sql="SELECT * FROM mop,customer_refund,currency where 
customer_refund.mop=mop.mop_id AND customer_refund.currency_code=currency.curr_id and 
customer_refund.date_paid BETWEEN '$date_from' AND '$date_to' and customer_refund.client='$client' order by customer_refund.date_paid  desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
...
echo $date_received=$rows->date_paid; 
	...
	$client=$rows->client_id;
	
$queryprod="select * from customer where customer_id='$client'";
$resultsprod=mysql_query($queryprod) or die ("Error: $queryprod.".mysql_error());
$rowsprod=mysql_fetch_object($resultsprod); 
echo $exp_cat_name=$rowsprod->customer_name;
...
else
{
	...
	<td align="center"><a href="edit_customer_refund.php?customer_refund_id=<?php echo $rows->customer_refund_id;?>"><img src="images/edit.png"></a></td>
    <td align="center"><a href="delete_customer_refund.php?customer_refund_id=<?php echo $rows->customer_refund_id;?>"  onClick="return confirmDelete();"><img src="images/delete.png"></a></td>
