<tr height="10" bgcolor="#C0D7E5" >
    <td width="10%" colspan="3">Long Term Loan</td>
...
from loans_ledger,currency where loans_ledger.currency_code=currency.curr_id order by loans_ledger.transaction_id desc";
...
else
{
	...
	$ledger_bal_ltm=$ledger_bal_ltm+$amount_kshs; 
  ...
  <?php 
  echo number_format($ledger_bal_ltm,2);
  }
...
$grnd_amount_out_kshs=0; 
$sql="select  loans_ledger.transaction_id,loans_ledger.transactions,loans_ledger.amount,loans_ledger.debit,loans_ledger.credit,loans_ledger.currency_code,loans_ledger.curr_rate,loans_ledger.date_recorded, currency.curr_name 
from loans_ledger,currency where loans_ledger.currency_code=currency.curr_id AND loans_ledger.date_recorded BETWEEN '$date_from' AND '$date_to' order by loans_ledger.transaction_id desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
