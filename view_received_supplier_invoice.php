...
$querydc="select * FROM received_supplier_invoice_code,order_code_get,suppliers WHERE 
suppliers.supplier_id=order_code_get.supplier_id and received_supplier_invoice_code.order_code_id=order_code_get.order_code_id 
ORDER BY received_supplier_invoice_code_id desc";
...
$querydc= "select * FROM received_supplier_invoice_code,order_code_get,suppliers";
    $conditions = array();
    ...
    $conditions[] = "received_supplier_invoice_code.supplier_id='$supplier'";
	...
	if($date_from!='' && $date_to!='' ) {
      ...
      $conditions[] = "received_supplier_invoice_code.date_paid >='$date_from' AND received_supplier_invoice_code.date_paid<='$date_to' ";
    }

    if (count($conditions) > 0) {
      $querydc .= " WHERE " . implode(' AND ', $conditions)." AND suppliers.supplier_id=order_code_get.supplier_id and received_supplier_invoice_code.order_code_id=order_code_get.order_code_id 
ORDER BY received_supplier_invoice_id desc";
    }
	else
	{	
	
$querydc="select * FROM received_supplier_invoice_code,order_code_get,suppliers WHERE suppliers.supplier_id=order_code_get.supplier_id and 
received_supplier_invoice_code.order_code_id=order_code_get.order_code_id 
ORDER BY received_supplier_invoice_code_id desc";
$resultsdc= mysql_query($querydc) or die ("Error $querydc.".mysql_error());
...
if (mysql_num_rows($resultsdc) > 0)
	...
	$id=$rows->received_supplier_invoice_code_id;
...
$stock_code_id=$rows->order_code_id;
 ?>
	...
	<td align="right"><?php echo number_format($ttl_order_amount=$rows->ttl_order_amount,2);?></td>
...
while ($rowsemp_det2=mysql_fetch_object($resultsemp_det2))
   ...
   ?></td>
   <td width="30%"><?php echo number_format($credit_amount=$rowsemp_det2->debit_amount,2);
   ...
   <?php //echo number_format($vat_amount=$rowsemp_det2->ttl_vat,2);
   
   $ttl_vat=$ttl_vat+$vat_amount;
...
<td><strong><?php echo number_format($rows->ttl_vat,2); ?></strong></td>
</tr>							  
...
$grnt_cost_kshs=$grnt_cost_kshs+$ttl_order_amount; 
...
$grnd_stock_bal=$grnd_stock_bal+$stock_bal;
  ...
  ?>
    ...
    <table width="100%" border="0">
  <tr height="30">
  <td><strong><font size="+1">Totals</font></strong></td>
  <td width="40%"><strong><font color="#ff0000" size="+1"><?php echo number_format($grnt_cost_kshs,2);?></font></strong></td>
  <td></td>
  <td></td>
  <td></td>
