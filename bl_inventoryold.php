...
$stock_ret=$rowsstockret->ttl_stock_ret;	
 ...
 //number_format($ttl_costxx=$avail_prod*$bp*$curr_rate,2);
 number_format($ttl_costxx=$avail_prod*$bp,2);
  ...
  <?php 
 $grnt_qnt_sold=$grnt_qnt_sold+$sold_prod;
 $grnt_qnt_soldcash=$grnt_qnt_soldcash+$prod_cash_sold;
  $grnt_amnt_ttl_ccostxx=$grnt_amnt_ttl_ccostxx+$ttl_costxx;
 
  }
  ?>
 
	<?php echo number_format($grnt_amnt_ttl_ccostxx,2);
...
$sqlret="select SUM(sales_returns.sales_return_quantity ) as ttl_sales_ret from sales_returns where sales_returns.product_id='$prod_id' and date_returned BETWEEN '$date_from' AND '$date_to'";
$resultsret= mysql_query($sqlret) or die ("Error $sqlret.".mysql_error());
$rowsret=mysql_fetch_object($resultsret);
...
$sqlstockret="select SUM(stock_returns.stock_return_quantity) as ttl_stock_ret from stock_returns where stock_returns.product_id='$prod_id' and date_returned BETWEEN '$date_from' AND '$date_to'";
$resultsstockret= mysql_query($sqlstockret) or die ("Error $sqlstockret.".mysql_error());
$rowsstockret=mysql_fetch_object($resultsstockret);
