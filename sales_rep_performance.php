...
<body>
	...
	<div id="page-wrap">
		...
		<h3>:: Sales Rep Performance Report (Commission Earned)</h3>
...
<table width="70%" border="0" style="margin:auto;">
  <tr bgcolor="#FFFFCC">
    ...
    <td colspan="10" height="30" align="center"> 
		<a target="_blank" style="float:right; margin-right:50px; font-weight:bold; font-size:13px; color:#000000" href="print_list_sales_rep_perf.php">Print</a>						  
...
<tr bgcolor="#FFFFCC">
	...
	<strong>Enter Sales Rep Name:  <input type="text" name="item_name" size="20"/>
...
<strong>Period From</strong>
  ...
  <td align="center" width="100"><strong>Sales Rep Code.</strong></td>    
    <td align="center" width="160"><strong>Sales Rep Name</strong></td>
	<td align="center" width="200"><strong>Commission Earned (Kshs)</strong></td>
...
$queryop="SELECT users.f_name,SUM(amount_earned) as ttl_days,users.user_code FROM commision_earned,users WHERE
users.user_id=commision_earned.sales_rep GROUP BY commision_earned.sales_rep order by ttl_days desc";
...
$queryop= "SELECT users.f_name,SUM(amount_earned) as ttl_days,users.user_code FROM commision_earned,users";
    $conditions = array();
	
if($client!='') 
 {
	
      $conditions[] = "users.f_name LIKE '%$client%'";
} 

 if($date_from!='' && $date_to!='' ) 
 {
	
      $conditions[] = "commision_earned.date_earned>='$date_from' AND commision_earned.date_earned <='$date_to'";
}
...
$queryop .= " WHERE " . implode(' AND ', $conditions)." AND users.user_id=commision_earned.sales_rep GROUP BY 
commision_earned.sales_rep order by ttl_days desc";
...
echo '<tr bgcolor="#FFFFCC" height="25" >';
 ...
 ?>
    ...
    <td><?php echo $region_id=$rows->user_code;?></td>
    <td>
...
*/
echo number_format($rows->ttl_days,2); 
