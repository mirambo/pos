...
<thead>	
 ...
 <td width="5%"><strong>No</strong></td>
  <td width="5%"><strong>Supplier Code</strong></td>
 <td width="15%"><strong>Supplier Name</strong></td>
  <td width="10%"><strong>LPO No</strong></td>
...
$queryop="SELECT * FROM order_code_get,suppliers,currency where order_code_get.currency=currency.curr_id and order_code_get.supplier_id=suppliers.supplier_id 
...
echo $rows->supplier_name;
 ...
 <td align=""><?php echo $rows->curr_name.' '; include('lpo_value.php') ?></td>
  <td align="center">
 <?php 
echo $sale_date=$rows->date_generated;
...
$ttl_loan=$lpo_ttl;
$task_totals=$lpo_ttl;
$queryopg2sr = "SELECT SUM(order_amount_received) as int_paid FROM supplier_payments where order_code_id='$order_code'";
$resultsg2sr= mysql_query($queryopg2sr) or die ("Error $queryopg2s.".mysql_error());	
$rowsg2sr=mysql_fetch_object($resultsg2sr); 

echo number_format($ttl_repayment_paid=$rowsg2sr->int_paid,2);
