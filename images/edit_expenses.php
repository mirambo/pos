...
$invoice_payment_id=$_GET['expenses_id'];
$sql="select * FROM mop,expenses,currency where 
expenses.mop=mop.mop_id AND expenses.currency_code=currency.curr_id 
 AND expenses.expenses_id='$invoice_payment_id'";
...
<form name="gen_order" action="processeditexp.php?invoice_payment_id=<?php echo $invoice_payment_id;?>" method="post">			
...
Exchange Rate To(Kshs)<input type="text" name="curr_rate" size="10" value="<?php echo $rows->curr_rate; ?>">							   
...
if (mysql_num_rows($resultsmop) > 0)
{
  ...
  <tr><td colspan="8" height="20" bgcolor="#FFFFCC" align="center"><strong>Cheque Payment Details(If paid via cheque )</strong></td></tr>
	...
	?>"/></td>
							   <td valign="top" colspan="2">Cheque Issuer<font color="#FF0000"></font>
...
if (mysql_num_rows($resultsbnk) > 0)
{
						while ($rowsbnk=mysql_fetch_object($resultsbnk))
							{						
                               ...
                               </select> Client Bank<input type="text" name="client_bank1" size="15" value="<?php 
	if ($mop==2)
	{
