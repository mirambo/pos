...
$queryprodsup="select * FROM salary_payments order by salary_payment_id desc limit 1";
...
$p_month=$rowsprodsup->month_paid;
...
<body>
	...
	<div id="page-wrap">
		...
		<h3>::List of All Approved Payslips Ready for salary payments</h3>
...
if ($_GET['finished']==1)
{
echo '<div align="center" style="background: #FFCC33; height:20px; width:350px; border:#FF0000 solid 1px; font-size:11px;" class="br-5"> <p align="center"><font color="#FF0000" >Salary Payment Processed Successfuly' ;?> <?php echo $invoice_no;?> <?php echo '</font></strong></p></div></br>';


echo '<div align="center" style="background: #000033; height:30px; width:350px; border:#FF0000 solid 1px; font-size:11px;" class="br-5"> <a href="view_payslip.php?month_pay='.$p_month.'"><font color="#FFffff" >Click Here To Print Payslips</a>' ;?> <?php echo $invoice_no;?> <?php echo '</font></strong></p></div>';
}
...
{
?>
<a href="final_salary_process.php" ><div style="float:right; width:400px; background:#ff0000;color:#ffffff; text-decoration:none; font-weight:bold; padding:3px; border-radius:5px;">Click here to Process The Salary Payments...</div></a>
<?php 
}
?>	
	</td>
...
<td width="100"><div align="center"><strong>Approved By</strong></td>
<td width="100"><div align="center"><strong>Print Payslip</strong></td>
...
{   
$sql="select * FROM payroll,employees,approved_payroll WHERE approved_payroll.payroll_id=payroll.payroll_id and 
...
employees.employee_no,employees.basic_sal FROM payroll,employees WHERE payroll.emp_id=employees.emp_id and payroll.status='1' 
AND payroll.emp_id='$emp' order by payroll.payroll_id desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
...
payroll.emp_id,payroll.net_pay,employees.emp_firstname,employees.emp_middle_name,employees.emp_lastname,
employees.employee_no,employees.basic_sal FROM payroll,employees WHERE payroll.emp_id=employees.emp_id and payroll.status='1' 
AND payroll.payment_month='$current_month' order by payroll.payroll_id desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
...
payroll.emp_id,payroll.net_pay,employees.emp_firstname,employees.emp_middle_name,employees.emp_lastname,
employees.employee_no,employees.basic_sal FROM payroll,employees WHERE payroll.emp_id=employees.emp_id and payroll.status='1' 
AND payroll.emp_id='$emp' AND payroll.payment_month='$current_month' order by payroll.payroll_id desc";
$results= mysql_query($sql) or die ("Error $sql.".mysql_error());
}
else
{
...
$sqlpv="select * from approved_payroll,users where  approved_payroll.approving_user_id=users.user_id 
AND approved_payroll.payroll_id='$payroll_id'";
$resultspv= mysql_query($sqlpv) or die ("Error $sqlpv.".mysql_error());
$rowspv=mysql_fetch_object($resultspv);
echo $rowspv->f_name;


	 ?></td>
	 <td width="200"><div align="center"><a href="view_payslip.php?payroll_id=<?php echo $payroll_id; ?>" target="_blank">Print Payslip</a></td>
    </tr>
