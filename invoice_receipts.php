...
where sales_receipt.client_id=clients.client_id AND sales_receipt.currency_code=currency.curr_id AND sales_receipt.sales_code=invoices.sales_code and sales_receipt.status='0' group by sales_receipt.receipt_no  order by sales_receipt.date_generated desc";
...
if ($mop=='Cheque')
{ ?>
<a href="sales_bounce_cheque.php?receipt_no=<?php echo $rows->receipt_no; ?>&sales_receipt_id=<?php echo $rows->sales_rep_id;?>
&amount_bounced=<?php echo $rows->amnt_rec;?>&currency=<?php echo $rows->currency_code;?>&curr_rate=<?php echo $rows->curr_rate; ?>
&client_id=<?php echo $rows->client_id; ?>&cheque_no=<?php echo $cheque_no;?>" onClick="return confirmDelete();">Cheque(No:<?php echo $cheque_no; ?>)</a
<?php 
}
else
{
