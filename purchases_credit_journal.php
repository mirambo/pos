...
$queryop2x = "select * FROM suppliers,purchases_debit_journal_code,currency where 
purchases_debit_journal_code.supplier_id=suppliers.supplier_id
and purchases_debit_journal_code.currency=currency.curr_id AND purchases_debit_journal_code.journal_code_id='$id'";
...
<script type="text/javascript">
    ...
    $.get('load_curr2.php?parent_cat12=' + $(this).val(), function(data) {
    $("#sub_cat12").html(data);
...
$(document).ready(function () {
    ...
    $("#addrow").on("click", function () {
        ...
        cols += '<td><select required id="account_from'+ counter +'" name="debit_account_id[]' + counter + '"  style="width:200px;"><option value="">Select..............................</option><?php $query1="select * from account_type  order by account_code asc";  $results1=mysql_query($query1) or die ("Error: $query1.".mysql_error());  if ($num_rows1=mysql_num_rows($results1)>0) { while ($rows1=mysql_fetch_object($results1)) {  ?><option value="<?php echo $rows1->account_type_id; ?>"><?php echo $rows1->account_code." - ".mysql_real_escape_string($rows1->account_type_name); ?></option><?php      } } ?></select><?php include('"account_from.php"');?></td>';
        cols += '<td><input required type="text" class="txt" name="amount[]' + counter + '"/> <input type="button"  class="ibtnDel btn btn-md btn-danger" value="-Remove"></td>';
...
<form name="emp" action="process_add_purchase_credit_journal.php?sub_module_id=<?php echo $sub_module_id; ?>&id=<?php echo $id; ?>" enctype="multipart/form-data" method="post">		
...
<input type="date" class="form-control" style="width:80%; padding:5px;" id="file_title" name="warrant_date"  required size="18" value="<?php echo $rows2x->journal_date; ?>" /></td>
	...
	<option value="<?php echo $rows2x->supplier_id; ?>"><?php echo $rows2x->supplier_name; ?></option>
    <?php 
    ...
    <option value="<?php echo $row['supplier_id']; ?>"><?php echo $row['sup_code'].' '.$row['supplier_name']; ?></option>
    <?php endwhile; ?>
    </select>
...
?> <font color="#FF0000">*</font></br><select name="currency" id="parent_cat12" required style="width:280px;">
...
<option value="<?php echo $rows2x->currency; ?>"><?php echo $rows2x->curr_name; ?></option>
  ...
  <tr height="10">
    ...
    <td width="20%" align=""><?php $english_text9="Transaction Description";
...
<input type="text" value="<?php echo $rows2x->transaction_description; ?>" name="transaction_desc" size="40">
    ...
    <td width="23%"  height="50" ><span id="sub_cat12">
							   Refference No: </br><input type="text" name="ref_no" value="<?php echo $rows2x->ref_no;?>" required size="30">
							   </span></td>
  ...
  <!--<tr height="10">
    ...
    <td width="20%" align=""><?php $english_text9="VAT";
...
<select name='vat' required style="width:280px;">
...
<td width="20%"><strong>Debit Amount</strong></th>
    ...
    <tbody>
	   ...
	   <?php 
$sqlemp_det2="select * FROM account_type,purchases_debit_journal_transaction where 
purchases_debit_journal_transaction.debit_account_id=account_type.account_type_id AND 
purchases_debit_journal_transaction.journal_code_id='$id'";
...
while ($rowsemp_det2=mysql_fetch_object($resultsemp_det2))
   ...
   ?>
	...
	<tr>
	<td>
	<input type="hidden" name="debit_journal_transaction_id[<?php echo $rowsemp_det2->journal_transaction_id; ?>]" value="<?php echo $rowsemp_det2->journal_transaction_id;?>">
...
while ($rowsemp_det2=mysql_fetch_object($resultsemp_det2))
   ...
   ?>
	...
	<input type="hidden" name="credit_journal_transaction_id[<?php echo $rowsemp_det2->journal_transaction_id; ?>]" value="<?php echo $rowsemp_det2->journal_transaction_id;?>">
