...
$invoice_payment_id=$_GET['supplier_refund_id'];
$sql="select * FROM mop,supplier_refund,currency where 
supplier_refund.mop=mop.mop_id AND supplier_refund.currency_code=currency.curr_id AND supplier_refund_id='$invoice_payment_id'";
...
$exp_cat_id=$rows->supplier_id;
$queryprod="select * from suppliers where supplier_id='$exp_cat_id'";
$resultsprod=mysql_query($queryprod) or die ("Error: $queryprod.".mysql_error());
$rowsprod=mysql_fetch_object($resultsprod); 
$exp_cat_name=$rowsprod->supplier_name;
...
<?php require_once('includes/supplier_refund_submenu.php');  ?>
		...
		<h3>:: Update Supplier Refund Details</h3>
...
<form name="gen_order" action="processedit_supplier_refund.php?invoice_payment_id=<?php echo $invoice_payment_id;?>" method="post">			
...
?></td>
    ...
    <td colspan="9" >Select Payment Date:<input type="text" name="date_paid" size="40" value="<?php echo $rows->date_paid; ?>" id="date_paid" readonly="readonly" /></td>
	</tr>
...
<select name="exp_cat_id" style="width:100px;">
	<option value="<?php echo $exp_cat_id ?>"><?php echo $exp_cat_name; ?></option>
								  <?php 
								  ...
								  
								  {
                                    ...
                                    <option value="<?php echo $rows1->exp_cat_id; ?>"><?php echo $rows1->supplier_name;?> </option>
